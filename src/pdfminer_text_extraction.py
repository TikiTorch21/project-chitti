...
from utils.pdf_utils import pdfminer_extract_pdf_text
...
def text_extract(pdf_dir):
    ...
    all_pdf_text = [
        pdfminer_extract_pdf_text(pdf) for pdf in pdf_dir
    ]
...
pdf_df.to_csv(PROJECT_PATH / 'data' / 'text extractions' / 'pdfminer.six' / 'pdfminer_txt_extractions.csv', index=None)