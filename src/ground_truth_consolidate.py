...
def merge_csvs(path1: str, path2: str) -> pd.DataFrame:
    ...
    final_df = pd.merge(
        extractions_df, ground_truth_df, left_on=left_col, right_on="pdfId", how="inner"
    )
    return final_df.drop("content_actual", axis=1)


TXT_EXTRACTION_PATH = Path(
    "/Users/prateekM/Downloads/Coding/Classes/Projects/Project Chitti/data/text extractions"
)

ground_truth_final = merge_csvs(
    TXT_EXTRACTION_PATH / "consolidated_extractions.csv",
    TXT_EXTRACTION_PATH / "ground_truth_txt_extractions.csv",
)


def add_library_similarity_scores(df, extraction_cols):
    d = {"1": "pypdf", "2": "pymupdf", "3": "pdfminer"}
