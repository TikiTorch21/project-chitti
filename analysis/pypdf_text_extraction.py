...
def text_extract(pdf_dir):
    ...
    all_pdf_text = [pypdf_extract_pdf_text(pdf) for pdf in pdf_dir]
    ...
    df.columns = [col + "_pypdf" for col in df.columns]
    return df
...
pdf_df.to_csv(
    PROJECT_PATH / "data" / "text extractions" / "pypdf" / "pypdf_txt_extractions.csv",
    index=None,
)
